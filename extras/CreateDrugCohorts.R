...
custom_definitions <- readr::read_csv("extras/drug_classes_sql.csv", col_types = readr::cols())
custom_definitions <- custom_definitions[-grep("all", custom_definitions$cohortName),]

### 2. remove some concepts from concept sets
# load in concepts to remove (selected by manual inspection of all concepts with missing dose forms AND searching for terms such as "rectal" / "topical" / "otic" / "nasal" / "medicated pad" / "tape" in concept names)
removed_concepts <- read.csv("extras/removed_concepts.csv", sep=",", stringsAsFactors = FALSE)
...
combi_concepts <- read.csv("extras/combi_concepts.csv", sep=",", stringsAsFactors = FALSE)
